...
fontLoader.load(
        ...
        (font) => {
            ...
            scene.add(text)
            text.rotation.y = -2.265
            text.position.set(-15.009, 20.952, 0)
...
const fog = new THREE.Fog('#251818', 1, 80)
scene.fog = fog
    //gui.addColor(fog, 'color').onChange((value) => fog.color.set(value)).name('FogColor')
    /**
...
let mixer = null
gltfLoader.load(
    '/models/monsters_Background/MonstersBackground3DAni.glb',
    (gltf) => {
...
const points = [{
        position: new THREE.Vector3(-25, 17.0, 0),
...
camera.position.set(-57.345, 10.952, -35.728)
...
controls.target.set(-18.009, 8.952, 0)
